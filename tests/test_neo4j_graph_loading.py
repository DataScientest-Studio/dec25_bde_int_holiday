...
def test_neo4j(neo4j_container):
    ...
    driver = GraphDatabase.driver(connection_url, auth=(neo4j_user, neo4j_password))
...
def _fetch_from_test_db(test_db):
    """Helper to create a fetch function that reads from the test database."""
    def fetch():
...
def test_load_pois_to_neo4j(test_db, test_neo4j):
    ...
    graph_loader.fetch_pois_from_postgres = _fetch_from_test_db(test_db)
